defmodule Testproject.TransportersTest do
  use Testproject.DataCase

  alias Testproject.Transporters

  describe "transporters" do
    alias Testproject.Transporters.Transporter

    import Testproject.TransportersFixtures
    ...
    test "create_transporter/1 with valid data creates a transporter" do
      valid_attrs = %{id: "some id", name: "some name", npwp_number: "some npwp_number", phone_number: "some phone_number", status: "some status"}

      assert {:ok, %Transporter{} = transporter} = Transporters.create_transporter(valid_attrs)
      assert transporter.id == "some id"
    ...
    test "update_transporter/2 with valid data updates the transporter" do
      transporter = transporter_fixture()
      update_attrs = %{id: "some updated id", name: "some updated name", npwp_number: "some updated npwp_number", phone_number: "some updated phone_number", status: "some updated status"}

      assert {:ok, %Transporter{} = transporter} = Transporters.update_transporter(transporter, update_attrs)
      assert transporter.id == "some updated id"
